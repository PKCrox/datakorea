...
에이전트 시대에 데이터가 핵심 인프라인 건 다 아는데, 한국 데이터가 실제로 어느 수준인지 보여주는 전체 그림이 없었다. data.go.kr은 공공만 다루고, 민간은 각자 포털에 흩어져 있고, 뭐가 열려 있고 뭐가 닫혀 있는지 한눈에 볼 방법이 없었다.

궁금해서 직접 세봤다. 공공 1,203개 기관 + 민간 151개 기업. 데이터셋이 몇 개고, API가 있는지, 접근하려면 뭘 해야 하는지.

사람은 대시보드로 보고, AI 에이전트는 `--json`으로 바로 쿼리하는 — **한국 데이터 어그리게이터**.
