...
솔직히 불안하다. 한국 데이터 인프라가 이 속도를 따라가고 있는지 확신이 안 선다.

- data.go.kr에 95,000개 데이터셋이 등록돼 있지만, API로 바로 접근 가능한 건 18%
- 민간 기업의 데이터 개방 수준은 아직 파악조차 되지 않았다
- 전체 그림을 볼 수 있는 곳이 없으니 판단 자체가 불가능했다

DataKorea는 그 판단을 하기 위해 만들었다. 공공 1,203개 기관, 민간 151개 기업의 데이터셋과 API를 전수 조사해서, **한국 데이터가 실제로 얼마나 열려 있는지** 현황을 보여준다.
